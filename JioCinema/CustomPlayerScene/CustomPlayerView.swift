//
//  CustomPlayerView.swift
//  CustomPlayer
//
//  Created by Shweta Adagale on 26/02/19.
//  Copyright © 2019 Shweta Adagale. All rights reserved.
//

import UIKit
import AVFoundation
import Foundation

protocol DelegateToUpdateProgressBar {
    func currentTimevalueChanged(newTime : Float)
}

class CustomPlayerView: UIView {
    weak var controlsView : ControlsView?
    
    @IBOutlet weak var playerControlView: UIView!
    var player : AVPlayer?
    var playerItem : AVPlayerItem?
    var url: URL?
    var timer: Timer!
    var delegate : DelegateToUpdateProgressBar?
        var myPreferedFocusView : UIView?
        override var preferredFocusEnvironments: [UIFocusEnvironment]{
            if let preferredView = myPreferedFocusView {
                    preferredView.alpha = 1
                return [preferredView]
            }
            return []
        }
    func configureView(url: URL, superView: UIView) {
        self.url = url
        playerItem = AVPlayerItem(url: url)
        player = AVPlayer(playerItem: playerItem)
        player?.play()
        //self.timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(self.updateProgressBar), userInfo: nil, repeats: true)
        playerControlView.isHidden = true
        let playerLayer = AVPlayerLayer(player: player)
        playerLayer.frame = self.bounds
        self.clipsToBounds = true
        self.layer.addSublayer(playerLayer)
        addControlsView()
                if self.controlsView != nil {
                    DispatchQueue.main.async {
                        self.myPreferedFocusView = self
        
                    }
                }
        self.updateFocusIfNeeded()
        self.setNeedsFocusUpdate()
    }
    func addControlsView() {
        let viewforControls = UINib(nibName: "ControlsView", bundle: .main).instantiate(withOwner: nil, options: nil).first as! ControlsView
        self.controlsView = viewforControls
        viewforControls.configureControls()
        viewforControls.delegate = self
        viewforControls.addAsSubviewWithFourConstraintsFromBottomWithConstantHeight(self, height: 300, bottom: -100, leading: 0, trailing: 0)
    }
    @objc func updateProgressBar() {
        if let duration = playerItem?.asset.duration, let currentTime = playerItem?.currentTime(){
            let newValue = currentTime.seconds / duration.seconds
            controlsView?.updateProgressBar(progress: Float(newValue))
        }
    }
    var start:CGPoint?
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if touches.count > 1 {
            return
        }
        controlsView?.touchBeganCalledSetSliderValue()
        if let touch = touches.first{
            start = touch.location(in: self.superview)
            print("Start:\(String(describing: start?.x))")
        }
    }
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if touches.count > 1 {
            return
        }
        if let touch = touches.first{
            start = touch.location(in: self.superview)
            print("End:\(String(describing: start?.x))")
        }
    }
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let touch = touches.first{
            //            if player?.rate == 0{
            let startingPoint: CGFloat = 960
            let maxDisplacement: CGFloat = 1920
            var displacement : CGFloat = 0.0
            //                if touch.location(in: self.superview).x < startingPoint{
            //                    let value = (touch.location(in: self.superview).x - startingPoint)
            //                    displacement = value
            //                } else if touch.location(in: self.superview).x > startingPoint{
            //                    displacement = (touch.location(in: self.superview).x - startingPoint)
            //                }
            displacement = (touch.location(in: self.superview).x - startingPoint)
            let scale = displacement / maxDisplacement
            controlsView?.updateProgressBar(progress: Float(scale/4), dueToScrubing: true)
            //            if let duration = playerItem?.asset.duration{
            ////            let totalSeconds = CMTimeGetSeconds(duration)
            ////
            ////            let changedTime = Float64(newValue) * totalSeconds
            //            var newchangedTime = Float64(player?.currentItem?.currentTime().value ?? 0)
            //            newchangedTime += changedTime
            //             let seekTime = CMTime(value: Int64(newchangedTime), timescale: 1)
            //                DispatchQueue.main.async {
            //                    self.player?.seek(to: seekTime, completionHandler: { (completedSeek) in
            //                        //perhaps do something later here
            //                    })
            //                }

            //            }

            //            }
        }
    }
    override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        print("touchesCancelled")
    }
}
extension CustomPlayerView : PlayerControlsDelegate {
    func playTapped(_ isPaused: Bool) {
        if !isPaused {
            player?.pause()
        } else {
            player?.play()
        }
    }
}
