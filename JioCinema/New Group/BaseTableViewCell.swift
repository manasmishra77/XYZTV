...
extension BaseTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didUpdateFocusIn context: UICollectionViewFocusUpdateContext, with coordinator: UIFocusAnimationCoordinator) {
        if let item = cellItems.items {
            ...
            if context.nextFocusedItem is ItemCollectionViewCell {
                delegate?.setHeaderValues(item: self, urlString: newItem.LogoUrlForDisneyChar, title: newItem.name ?? "", subtitle: nil, maturityRating: nil, description: "", toFullScreen: false, mode: .scaleAspectFit)
            }
        }
    }
}
